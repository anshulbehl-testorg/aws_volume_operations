Aws.Aws_volume_operations Run Role
========================
...
- name: Execute tasks on servers
  hosts: servers
  roles:
    - role: aws.aws_volume_operations.run
      run_x: 42
```
...
```yaml
- name: Initialize the run role from aws.aws_volume_operations
  hosts: servers
  gather_facts: false
  tasks:
    - name: Trigger invocation of run role
      ansible.builtin.include_role:
        name: aws.aws_volume_operations.run
      vars:
        run_x: 42
```
